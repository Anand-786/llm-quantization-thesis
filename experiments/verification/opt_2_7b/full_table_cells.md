# Verification: Full WikiText-2 PPL table — OPT-2.7B
...
**Prereqs on Drive:**
- `act_scales/opt-2.7b.pt` — for max-based smoothing (rows 3,4,6,7,8) and α(l) severity build (rows 7,8).
- `act_percentiles/opt-2.7b/p0.995.pt` — for row 5 (Task 02 winner).

**Hardware:** A100 (T4 also fits OPT-2.7B but each PPL run is ~10-15 min, so 8 runs end-to-end on T4 ≈ 80-120 min; A100 ≈ 40-60 min).
...
import os, shutil
SAVE_DIR = "/content/drive/MyDrive/thesis_results/verification/opt-2.7b"
...
PCT_PATH = "/content/drive/MyDrive/thesis_results/act_percentiles/opt-2.7b/p0.995.pt"
...
Same helper as `experiments/task02_percentile_smoothing/opt_2_7b/percentile_sweep_cells.md`, copied here so this notebook is self-contained.
...
Identical formula to `experiments/task05_per_layer_alpha/opt_2_7b/per_layer_alpha_cells.md`.
...
# Task 02 winner for OPT-2.7B: p=0.995, α=0.5
...
print(f"\n{'='*70}")
print(f"  OPT-2.7B — WikiText-2 PPL verification (single session)")
print(f"{'='*70}")
...
print(f"  C/pct p=0.995 α=0.5 − FP16 = {ppl(f'5_C_pct{PCT_P}_a{PCT_A}') - fp16_ppl:+.4f}")
...
- **Naive W8A8** here is `weight_quant=per_tensor, act_quant=per_tensor` with no smoothing — the strictest "no-recipe" baseline. If you want the per-token-A flavor of naive instead, swap `NAIVE_QPARAMS` to use `act_quant="per_token"`.
- **Row 5 (Task 02 winner)** uses p=0.995, α=0.5 — the OPT-2.7B winner from `project_task02_opt2_7b_results.md` (12.34 PPL ≈ FP16). Change `PCT_P` / `PCT_A` if you want a different cell of the Task 02 grid.
- **α(l) build (Cell 3)** must run before Cell 4 — the `alpha_per_layer` list is consumed in the `RUNS` table.
- All 8 results land at `/content/drive/MyDrive/thesis_results/verification/opt-2.7b/` as one JSON per run plus a combined `opt-2.7b_summary.json`.
