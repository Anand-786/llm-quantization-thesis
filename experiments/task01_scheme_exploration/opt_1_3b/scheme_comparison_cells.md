# Task 01: Scheme Comparison — Colab Cells
...
!python run_scheme_compare.py \
    ...
    --config_label FP16 \
    --save_json results/task01/opt-1.3b_FP16.json
```
...
!python run_scheme_compare.py \
    ...
    --save_json results/task01/opt-1.3b_W8A8-naive.json
```
...
!python run_scheme_compare.py \
    ...
    --config_label SQ-O1 \
    --save_json results/task01/opt-1.3b_SQ-O1.json
```
...
!python run_scheme_compare.py \
    ...
    --weight_quant per_tensor --act_quant per_tensor \
    --config_label SQ-O2 \
    --save_json results/task01/opt-1.3b_SQ-O2.json
```
...
!python run_scheme_compare.py \
    ...
    --save_json results/task01/opt-1.3b_SQ-PCW-PT.json
```
...
!python run_scheme_compare.py \
    ...
    --save_json results/task01/opt-1.3b_SQ-PCW-TEN.json
```
...
print("-" * 35)
for f in sorted(glob.glob("results/task01/opt-1.3b_*.json")):
    r = json.load(open(f))
...
-----------------------------------
FP16            14.6278   390.0s
W8A8-naive      15.xxxx   xxx.xs
SQ-O1           14.xxxx   xxx.xs
SQ-O2           14.xxxx   xxx.xs
SQ-PCW-PT       14.xxxx   xxx.xs
SQ-PCW-TEN      14.xxxx   xxx.xs
```
