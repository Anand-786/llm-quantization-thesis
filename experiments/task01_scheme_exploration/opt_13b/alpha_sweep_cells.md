# Task 01: Alpha Sweep — O1 vs C on OPT-13B
...
10 total runs. Run Cell 1 (setup), then Cell 2 (sweep) and walk away.

**Hardware**: requires Colab Pro **A100 (40 GB)**. OPT-13B is ~26 GB in FP16; the sweep reloads the model each run (smoothing edits weights in-place), so each iteration spends ~1 min loading from disk before the eval.
...
SCALES = "/content/llm-quantization-thesis/smoothquant_repo/act_scales/opt-13b.pt"
...
for alpha in ALPHAS:
    for config_label, qparams in CONFIGS.items():
        ...
        fname = f"opt-13b_{config_label}_a{alpha}.json"
...
with open(os.path.join(SAVE_DIR, "all_results_opt-13b.json"), "w") as f:
    json.dump(all_results, f, indent=2)

# --- Print summary table ---
print(f"\n\n{'='*60}")
print(f"  ALPHA SWEEP RESULTS — OPT-13B")
print(f"{'='*60}")
