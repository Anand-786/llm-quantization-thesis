# Task 01: Scheme Comparison — Colab Cells (OPT-13B)

Copy-paste each cell into Colab. Run in order.

**Hardware**: requires Colab Pro **A100 (40 GB)**. OPT-13B is ~26 GB in FP16 — does not fit on a T4. Keep `seq_len=2048` for PPL comparability with the rest of the ladder.

Prerequisite: `opt-13b.pt` already generated and on Drive at `/content/drive/MyDrive/thesis_results/act_scales/` (see `generate_act_scales_cells.md`).
...
!python run_scheme_compare.py \
    ...
    --act_scales_path smoothquant_repo/act_scales/opt-13b.pt \
    --config_label FP16 \
    --save_json results/task01/opt-13b_FP16.json
```
...
!python run_scheme_compare.py \
    ...
    --save_json results/task01/opt-13b_W8A8-naive.json
```
...
!python run_scheme_compare.py \
    ...
    --config_label SQ-O1 \
    --save_json results/task01/opt-13b_SQ-O1.json
```
...
!python run_scheme_compare.py \
    ...
    --weight_quant per_tensor --act_quant per_tensor \
    --config_label SQ-O2 \
    --save_json results/task01/opt-13b_SQ-O2.json
```
...
!python run_scheme_compare.py \
    ...
    --save_json results/task01/opt-13b_SQ-PCW-PT.json
```
...
!python run_scheme_compare.py \
    ...
    --save_json results/task01/opt-13b_SQ-PCW-TEN.json
```
...
print("-" * 35)
for f in sorted(glob.glob("results/task01/opt-13b_*.json")):
    r = json.load(open(f))
...
OPT-13B is the largest model in the ladder. The paper reports per-tensor activation collapses badly at this scale (Table 1), so expect W8A8-naive and SQ-O2 to be much worse than at 6.7B, while C should remain close to FP16.
