# Task 01: Scheme Comparison — Colab Cells (OPT-6.7B)

Copy-paste each cell into Colab. Run in order.

Prerequisite: `opt-6.7b.pt` already generated and on Drive at `/content/drive/MyDrive/thesis_results/act_scales/` (see `generate_act_scales_cells.md`).
...
!python run_scheme_compare.py \
    ...
    --act_scales_path smoothquant_repo/act_scales/opt-6.7b.pt \
    --config_label FP16 \
    --save_json results/task01/opt-6.7b_FP16.json
```
...
!python run_scheme_compare.py \
    ...
    --save_json results/task01/opt-6.7b_W8A8-naive.json
```
...
!python run_scheme_compare.py \
    ...
    --config_label SQ-O1 \
    --save_json results/task01/opt-6.7b_SQ-O1.json
```
...
!python run_scheme_compare.py \
    ...
    --weight_quant per_tensor --act_quant per_tensor \
    --config_label SQ-O2 \
    --save_json results/task01/opt-6.7b_SQ-O2.json
```
...
!python run_scheme_compare.py \
    ...
    --save_json results/task01/opt-6.7b_SQ-PCW-PT.json
```
...
!python run_scheme_compare.py \
    ...
    --save_json results/task01/opt-6.7b_SQ-PCW-TEN.json
```
...
print("-" * 35)
for f in sorted(glob.glob("results/task01/opt-6.7b_*.json")):
    r = json.load(open(f))
...
If C < O1 and D < O2 (lower PPL = better), the hypothesis holds. Paper Table 1 shows per-tensor activation collapses at 6.7B+ (39.9% accuracy vs 64.8% per-channel), so the W8A8-naive gap is expected to be large.
