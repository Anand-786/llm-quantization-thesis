# Task 02: Generate Activation Percentiles — OPT-125M (A100)
...
Output: one `.pt` per `p` at `/content/drive/MyDrive/thesis_results/act_percentiles/opt-125m/p<value>.pt`. Each file is a `dict[name -> tensor[in_features]]` matching the shape of `act_scales/opt-125m.pt`. We also compute `p=1.0` from the same buffer in-memory and diff it against the existing `act_scales/opt-125m.pt` as a pipeline-correctness check, but **do not save** that row — fp16 storage in the buffer perturbs the per-channel max by up to ~0.5% relative (one fp16 ULP), which would muddy the sweep's `p=1.0` row. The real max-smoothing baseline is the Task 01 result.

Hardware: A100 (Colab Pro or JarvisLabs). Memory peak is trivial — model fp16 ≈ 250 MB, top-K buffers fp16 ≈ 1 GB across 24 OPT-125M smoothing sites (12 layers × {q_proj, fc1}, in_features = 768).
...
!mkdir -p /content/drive/MyDrive/thesis_results/act_percentiles/opt-125m

# Verify
!nvidia-smi
!ls -la /content/drive/MyDrive/thesis_results/act_scales/opt-125m.pt
...
We pass `p=1.0` into `get_act_percentiles` to get the per-channel max from the top-K buffer for an in-memory diff against the existing `act_scales/opt-125m.pt`. That gives us the pipeline-correctness check, but the `p=1.0` row itself is **not saved** — fp16 storage in the buffer perturbs it by up to ~0.5% relative (one fp16 ULP), and that noise is enough to confound the sweep's `p=1.0` row. The real reference baseline lives in Task 01's max-smoothing results.
...
ORIG_MAX_PATH = "/content/drive/MyDrive/thesis_results/act_scales/opt-125m.pt"
DATASET_PATH = "/content/llm-quantization-thesis/smoothquant_repo/dataset/val.jsonl.zst"
OUT_DIR = "/content/drive/MyDrive/thesis_results/act_percentiles/opt-125m"
...
act_pct = get_act_percentiles(
    ...
    num_samples=512,
    seq_len=512,
    buffer_device="cuda",        # OPT-125M buffers ~1 GB on GPU, trivial
    buffer_dtype=torch.float16,
)
...
- **Buffer size**: `K = ⌈0.10 · 262144⌉ + 16 = 26 230` rows × `in_features=768` per site, fp16. Total across 24 OPT-125M sites ≈ 1 GB GPU memory.
- **Hooked sites only**: 24 linears (12 layers × {q_proj, fc1}). out_proj and fc2 inputs aren't smoothed, so we skip them.
