# Task 02: Percentile Sweep — C scheme on OPT-125M (A100)
...
- 25 runs total, ~30 s/run on A100 → ~13 min end-to-end.
...
Prereq: run [generate_act_percentiles_cells.md](generate_act_percentiles_cells.md) once first to produce `act_percentiles/opt-125m/p<value>.pt` files on Drive.
...
!mkdir -p /content/drive/MyDrive/thesis_results/task02_percentile_sweep/opt-125m

# Verify all per-p scale files are present
!nvidia-smi
!ls -la /content/drive/MyDrive/thesis_results/act_percentiles/opt-125m/
...
PCT_DIR = "/content/drive/MyDrive/thesis_results/act_percentiles/opt-125m"
SAVE_DIR = "/content/drive/MyDrive/thesis_results/task02_percentile_sweep/opt-125m"
...
# fast if any are missing rather than several runs in.
...
for p in PERCENTILES:
    ...
    for alpha in ALPHAS:
        ...
        fname = f"opt-125m_C_p{p:g}_a{alpha}.json"
...
```python
print(f"\n{'='*70}")
print(f"  PERCENTILE SWEEP — OPT-125M, Scheme C")
print(f"{'='*70}")
...
To compare against max smoothing, look up the corresponding number from Task 01 (`results/task01/opt_125m/alpha_sweep_results.ipynb`) — both the C/max row at the same alpha and the paper's O1/max row.
...
- **Per-p .pt files are loaded one at a time** (then freed before the next `p`) so peak CPU memory stays small. Each file is ~0.3-0.5 MB so the load is essentially free.
