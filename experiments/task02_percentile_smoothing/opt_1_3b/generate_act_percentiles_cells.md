# Task 02: Generate Activation Percentiles — OPT-1.3B (A100)
...
Output: one `.pt` per `p` at `/content/drive/MyDrive/thesis_results/act_percentiles/opt-1.3b/p<value>.pt`. Each file is a `dict[name -> tensor[in_features]]` matching the shape of `act_scales/opt-1.3b.pt`. We also compute `p=1.0` from the same buffer in-memory and diff it against the existing `act_scales/opt-1.3b.pt` as a pipeline-correctness check, but **do not save** that row — fp16 storage in the buffer perturbs the per-channel max by up to ~0.5% relative (one fp16 ULP), which would muddy the sweep's `p=1.0` row. The real max-smoothing baseline is the Task 01 result.

Hardware: A100 (Colab Pro or JarvisLabs). GPU memory peak ≈ 12-14 GB (model fp16 ≈ 2.6 GB, top-K buffers fp16 ≈ 5 GB, activations + interpolation scratch).
...
!mkdir -p /content/drive/MyDrive/thesis_results/act_percentiles/opt-1.3b

# Verify
!nvidia-smi
!ls -la /content/drive/MyDrive/thesis_results/act_scales/opt-1.3b.pt
...
We pass `p=1.0` into `get_act_percentiles` to get the per-channel max from the top-K buffer for an in-memory diff against the existing `act_scales/opt-<size>.pt`. That gives us the pipeline-correctness check, but the `p=1.0` row itself is **not saved** — fp16 storage in the buffer perturbs it by up to ~0.5% relative (one fp16 ULP), and that noise is enough to confound the sweep's `p=1.0` row. The real reference baseline lives in Task 01's max-smoothing results.
...
ORIG_MAX_PATH = "/content/drive/MyDrive/thesis_results/act_scales/opt-1.3b.pt"
DATASET_PATH = "/content/llm-quantization-thesis/smoothquant_repo/dataset/val.jsonl.zst"
OUT_DIR = "/content/drive/MyDrive/thesis_results/act_percentiles/opt-1.3b"
...
act_pct = get_act_percentiles(
    ...
    num_samples=512,
    seq_len=512,
    buffer_device="cuda",        # <=6.7B fits on GPU
    buffer_dtype=torch.float16,
)
...
Expected pattern: as `p` decreases (1.0 → 0.90), the per-channel max in each file should also decrease (we're reading lower in the sorted distribution).

---

## Notes

- **Buffer size**: `K = ⌈0.10 · 262144⌉ + 16 = 26 230` rows × `in_features` per site, fp16. Total across 48 OPT-1.3B sites ≈ 5 GB GPU memory.
- **Hooked sites only**: 48 linears (24 layers × {q_proj, fc1}). out_proj and fc2 inputs aren't smoothed, so we skip them — saves both GPU memory and hook time.
- **Reproducibility**: `dataset.shuffle(seed=42)` matches the upstream `get_act_scales`. Calibration sample order is identical to Task 01's max scales, which is what makes Cell 4's diff meaningful.
- **fp16 precision**: per-channel scales feed an `alpha`-power formula; fp16 worst-case round-off (~10⁻³ relative) is well below the differences between consecutive percentiles we care about.
- **For 6.7B**: same script, `buffer_device="cuda"`, peak ≈ 30 GB on A100.
- **For 13B**: switch to `buffer_device="cpu"` so the GPU has room for the model and activations.
