# Task 02: Percentile Sweep — C scheme on OPT-2.7B (A100)
...
- 15 runs total, ~5-7 min/run on A100 → ~75-100 min end-to-end.
...
Prereq: run [generate_act_percentiles_cells.md](generate_act_percentiles_cells.md) once first to produce `act_percentiles/opt-2.7b/p<value>.pt` files on Drive.
...
!mkdir -p /content/drive/MyDrive/thesis_results/task02_percentile_sweep/opt-2.7b

# Verify all per-p scale files are present
!nvidia-smi
!ls -la /content/drive/MyDrive/thesis_results/act_percentiles/opt-2.7b/
...
PCT_DIR = "/content/drive/MyDrive/thesis_results/act_percentiles/opt-2.7b"
SAVE_DIR = "/content/drive/MyDrive/thesis_results/task02_percentile_sweep/opt-2.7b"
...
# fast if any are missing rather than an hour in.
...
for p in PERCENTILES:
    ...
    for alpha in ALPHAS:
        ...
        fname = f"opt-2.7b_C_p{p:g}_a{alpha}.json"
...
```python
print(f"\n{'='*70}")
print(f"  PERCENTILE SWEEP — OPT-2.7B, Scheme C")
print(f"{'='*70}")
...
To compare against max smoothing, look up the corresponding number from Task 01 (`results/task01/opt_2_7b/alpha_sweep_results.ipynb` once that exists) — both the C/max row at the same alpha and the paper's O1/max row.
...
- **Per-p .pt files are loaded one at a time** (then freed before the next `p`) so peak CPU memory stays small. Each file is ~5-7 MB so the load is essentially free.
