# Task 02: Generate Activation Percentiles — OPT-6.7B (A100)
...
Output: one `.pt` per `p` at `/content/drive/MyDrive/thesis_results/act_percentiles/opt-6.7b/p<value>.pt`. Each file is a `dict[name -> tensor[in_features]]` matching the shape of `act_scales/opt-6.7b.pt`. We also compute `p=1.0` from the same buffer in-memory and diff it against the existing `act_scales/opt-6.7b.pt` as a pipeline-correctness check, but **do not save** that row — fp16 storage in the buffer perturbs the per-channel max by up to ~0.5% relative (one fp16 ULP), which would muddy the sweep's `p=1.0` row. The real max-smoothing baseline is the Task 01 result.

Hardware: **A100 required** (T4 cannot fit OPT-6.7B at seq_len=2048 — see CLAUDE.md challenge #2). Memory peak during calibration ≈ 27 GB on A100-40 (model fp16 ≈ 13.3 GB, top-K buffers fp16 ≈ 13 GB across 64 OPT-6.7B smoothing sites — 32 layers × {q_proj, fc1}, in_features = 4096 — activations + interpolation scratch). Use A100-80 if available for headroom.

**Prereq**: the upstream max scales `act_scales/opt-6.7b.pt` must already be on Drive (we diff against them in Cell 3). If not, generate them first via the corresponding Task 01 procedure for OPT-6.7B.
...
!mkdir -p /content/drive/MyDrive/thesis_results/act_percentiles/opt-6.7b

# Verify
!nvidia-smi
!ls -la /content/drive/MyDrive/thesis_results/act_scales/opt-6.7b.pt
...
We pass `p=1.0` into `get_act_percentiles` to get the per-channel max from the top-K buffer for an in-memory diff against the existing `act_scales/opt-6.7b.pt`. That gives us the pipeline-correctness check, but the `p=1.0` row itself is **not saved** — fp16 storage in the buffer perturbs it by up to ~0.5% relative (one fp16 ULP), and that noise is enough to confound the sweep's `p=1.0` row. The real reference baseline lives in Task 01's max-smoothing results.
...
ORIG_MAX_PATH = "/content/drive/MyDrive/thesis_results/act_scales/opt-6.7b.pt"
DATASET_PATH = "/content/llm-quantization-thesis/smoothquant_repo/dataset/val.jsonl.zst"
OUT_DIR = "/content/drive/MyDrive/thesis_results/act_percentiles/opt-6.7b"
...
act_pct = get_act_percentiles(
    ...
    num_samples=512,
    seq_len=512,
    buffer_device="cuda",        # ~13 GB buffers + ~13 GB model fits A100-40
    buffer_dtype=torch.float16,
)
...
# Threshold rationale: at 6.7B scale, the per-channel max can drift up to a few
# percent between runs because (a) outlier channels reach fp16 magnitudes where
# one ULP is ~1-2% of the value, and (b) OPT's attention is not bit-deterministic
# on CUDA, so the same calibration on the same data can pick a slightly different
# fp16-representable max at outlier channels. We DO NOT save p=1.0 anyway — only
# percentile rows (p ≤ 0.999), which are read from rank p·(N-1) of a 262k-sample
# sorted buffer and are statistically robust to top-1 drift.
if max_rel_diff > 5e-2:
    raise RuntimeError(
        f"Top-K p=1.0 rel diff {max_rel_diff:.4f} exceeds 5% — likely a real bug, investigate."
    )
print(f"  rel diff is within the expected 1-2% band at this scale; percentile rows are unaffected.")
...
- **Buffer size**: `K = ⌈0.10 · 262144⌉ + 16 = 26 230` rows × `in_features=4096` per site, fp16. Total across 64 OPT-6.7B sites ≈ 13 GB GPU memory. Plus the fp16 model (~13.3 GB), peak is ~27 GB on A100-40.
- **A100-80 is preferred** for headroom but A100-40 fits. T4 is not an option for any OPT-6.7B work at seq_len=2048 — see CLAUDE.md challenge #2.
